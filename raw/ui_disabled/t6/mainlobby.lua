...
CoD.MainLobby.OnlinePlayAvailable = function (MainLobbyWidget, ClientInstance, Boolean)
	...
	if CoD.isPC and Engine.IsVacBanned() then
		...
		ErrorPopup:setMessage("Você foi banido pelo VAC e não pode jogar online.")
	...
	elseif UIExpression.IsGuest(ClientInstance.controller) == 1 then
		local ErrorPopup = MainLobbyWidget:openPopup("Error", ClientInstance.controller)
		ErrorPopup:setMessage("Convidados não podem usar este recurso.")
		ErrorPopup.anyControllerAllowed = true
	elseif UIExpression.DvarBool(ClientInstance.controller, "live_betaexpired") == 1 then
		local ErrorPopup = MainLobbyWidget:openPopup("Error", ClientInstance.controller)
		ErrorPopup:setMessage("A versão beta foi encerrada.")
	...
	elseif (UIExpression.IsContentRatingAllowed(ClientInstance.controller) == 0 or UIExpression.IsAnyControllerMPRestricted() == 1) and not Boolean then
		local ErrorPopup = MainLobbyWidget:openPopup("Error", ClientInstance.controller)
		ErrorPopup:setMessage("Este perfil não tem permissão para jogar online.")
		ErrorPopup.anyControllerAllowed = true
	elseif UIExpression.IsDemonwareFetchingDone(ClientInstance.controller) == 1 then
		...
		if PlayerStatTable1.cacLoadouts.resetWarningDisplayed:get() == 0 then
			...
			local ErrorPopup = MainLobbyWidget:openPopup("Error", ClientInstance.controller)
			ErrorPopup:setMessage("Suas estatísticas foram redefinidas devido a uma atualização. Suas classes foram preservadas.")
			ErrorPopup.anyControllerAllowed = true
		elseif CoD.isZombie == false and PlayerStatTable2.cacLoadouts.classWarningDisplayed:get() == 0 then
			PlayerStatTable2.cacLoadouts.classWarningDisplayed:set(1)
			local ErrorPopup = MainLobbyWidget:openPopup("Error", ClientInstance.controller)
			ErrorPopup:setMessage("Suas classes personalizadas foram redefinidas devido a uma atualização.")
			ErrorPopup.anyControllerAllowed = true
		else
			return 1
		end
...
CoD.MainLobby.IsControllerCountValid = function (MainLobbyWidget, LocalClientIndex, MaxLocalPlayers)
	if MaxLocalPlayers < UIExpression.GetUsedControllerCount() then
		...
		ErrorPopup:setMessage("Muitos controles conectados.")
		ErrorPopup.anyControllerAllowed = true
		return 0
	else
		return 1
...
CoD.MainLobby.OpenTheaterLobby = function (MainLobbyWidget, ClientInstance)
	...
	elseif UIExpression.CanSwitchToLobby(ClientInstance.controller, Dvar.party_maxplayers_theater:get(), Dvar.party_maxlocalplayers_theater:get()) == 0 then
		Dvar.ui_errorTitle:set("AVISO")
		Dvar.ui_errorMessage:set("Máximo de jogadores locais atingido para o Teatro.")
...
CoD.MainLobby.OpenStore = function (MainLobbyWidget, ClientInstance)
	...
	if CoD.MainLobby.OnlinePlayAvailable(MainLobbyWidget, ClientInstance, true) == 1 then
		if not CoD.isPS3 or UIExpression.IsSubUser(ClientInstance.controller) ~= 1 then
			...
			local ErrorPopup = MainLobbyWidget:openPopup("Error", ClientInstance.controller)
			ErrorPopup:setMessage("Sub-contas não podem acessar a loja.")
			ErrorPopup.anyControllerAllowed = true
		end
	end
end
...
CoD.MainLobby.PopulateButtons_Multiplayer = function (MainLobbyButtonPane)
	MainLobbyButtonPane.body.serverBrowserButton = MainLobbyButtonPane.body.buttonList:addButton("JOGAR ONLINE", nil, 1)
	MainLobbyButtonPane.body.serverBrowserButton.hintText = "Jogue partidas ranqueadas, não ranqueadas, com apostas e muito mais!"
	...
	if Engine.IsBetaBuild() then
		MainLobbyButtonPane.body.matchmakingButton = MainLobbyButtonPane.body.buttonList:addButton("PARTIDA RÁPIDA", nil, 3)
	else
		MainLobbyButtonPane.body.matchmakingButton = MainLobbyButtonPane.body.buttonList:addButton("PARTIDA RÁPIDA", nil, 2)
	end
	MainLobbyButtonPane.body.matchmakingButton.hintText = "Jogue partidas ranqueadas e não ranqueadas online."
	...
	if not Engine.IsBetaBuild() then
		MainLobbyButtonPane.body.customGamesButton = MainLobbyButtonPane.body.buttonList:addButton("PARTIDAS PERSONALIZADAS", nil, 4)
		MainLobbyButtonPane.body.customGamesButton.hintText = "Crie suas próprias partidas personalizadas com regras únicas."
	...
	MainLobbyButtonPane.body.theaterButton = MainLobbyButtonPane.body.buttonList:addButton("TEATRO", nil, 5)
	MainLobbyButtonPane.body.theaterButton:setActionEventName("open_theater_lobby")
	MainLobbyButtonPane.body.theaterButton.hintText = "Assista às suas melhores partidas ou veja destaques da comunidade."
	MainLobbyButtonPane.body.postTheaterSpacer = MainLobbyButtonPane.body.buttonList:addSpacer(CoD.CoD9Button.Height / 2, 6)
	if not Engine.IsBetaBuild() then
		MainLobbyButtonPane.body.barracksButton = MainLobbyButtonPane.body.buttonList:addButton("QUARTEL", nil, 7)
		MainLobbyButtonPane.body.barracksButton.id = "CoD9Button" .. "." .. "MainLobby" .. "." .. "QUARTEL"
	...
	if CoD.isZombie == false and not Engine.IsBetaBuild() and (CoD.isXBOX or CoD.isPS3) and Engine.IsEliteAvailable() and Engine.IsEliteButtonAvailable() then
		MainLobbyButtonPane.body.eliteAppButton = MainLobbyButtonPane.body.buttonList:addButton("ELITE", nil, 8)
		MainLobbyButtonPane.body.eliteAppButton.hintText = "Acompanhe suas estatísticas detalhadas no Call of Duty Elite."
	...
	MainLobbyButtonPane.body.optionsButton = MainLobbyButtonPane.body.buttonList:addButton("OPÇÕES", nil, 11)
	MainLobbyButtonPane.body.optionsButton.hintText = "Edite as configurações de jogo, áudio e vídeo."
	MainLobbyButtonPane.body.optionsButton:setActionEventName("open_options_menu")
	MainLobbyButtonPane.body.modsButton = MainLobbyButtonPane.body.buttonList:addButton("MODS", nil, 12)
...
CoD.MainLobby.PopulateButtons_Zombie = function (MainLobbyButtonPane)
	MainLobbyButtonPane.body.serverBrowserButton = MainLobbyButtonPane.body.buttonList:addButton("JOGAR ONLINE", nil, 1)
	MainLobbyButtonPane.body.serverBrowserButton.hintText = "Jogue partidas de zumbis online com outros jogadores."
	MainLobbyButtonPane.body.serverBrowserButton:setActionEventName("open_server_browser_mainlobby")
	CoD.SetupMatchmakingLock(MainLobbyButtonPane.body.serverBrowserButton)
	MainLobbyButtonPane.body.soloPlayButton = MainLobbyButtonPane.body.buttonList:addButton("JOGAR SOZINHO", nil, 3)
	MainLobbyButtonPane.body.soloPlayButton.hintText = "Enfrente sozinho as hordas de mortos-vivos e prove que você é o melhor matador de zumbis!"
	...
	MainLobbyButtonPane.body.customGamesButton = MainLobbyButtonPane.body.buttonList:addButton("PARTIDAS PERSONALIZADAS", nil, 4)
	MainLobbyButtonPane.body.customGamesButton.hintText = "Aqui é você quem manda! Jogue com outros sobreviventes contra hordas intermináveis de mortos-vivos!"
	MainLobbyButtonPane.body.customGamesButton:setActionEventName("open_custom_games_lobby")
	CoD.SetupCustomGamesLock(MainLobbyButtonPane.body.customGamesButton)
	MainLobbyButtonPane.body.theaterButton = MainLobbyButtonPane.body.buttonList:addButton("TEATRO", nil, 5)
	MainLobbyButtonPane.body.theaterButton:setActionEventName("open_theater_lobby")
	MainLobbyButtonPane.body.theaterButton.hintText = "Reveja suas partidas de zumbis e compartilhe seus melhores momentos."
	MainLobbyButtonPane.body.theaterSpacer = MainLobbyButtonPane.body.buttonList:addSpacer(CoD.CoD9Button.Height / 2, 6)
	MainLobbyButtonPane.body.barracksButton = MainLobbyButtonPane.body.buttonList:addButton("CLASSIFICAÇÕES", nil, 7)
	CoD.SetupBarracksLock(MainLobbyButtonPane.body.barracksButton)
	MainLobbyButtonPane.body.barracksButton:setActionEventName("open_barracks")
	MainLobbyButtonPane.body.optionSpacer = MainLobbyButtonPane.body.buttonList:addSpacer(CoD.CoD9Button.Height / 2, 8)
	MainLobbyButtonPane.body.optionsButton = MainLobbyButtonPane.body.buttonList:addButton("OPÇÕES", nil, 9)
	MainLobbyButtonPane.body.optionsButton.hintText = "Edite as configurações de jogo, áudio e vídeo."
	MainLobbyButtonPane.body.optionsButton:setActionEventName("open_options_menu")
	MainLobbyButtonPane.body.modsButton = MainLobbyButtonPane.body.buttonList:addButton("MODS", nil, 10)
	MainLobbyButtonPane.body.modsButton:setActionEventName("open_mods_menu")
end
...
CoD.MainLobby.PopulateButtons = function (MainLobbyButtonPane)
	...
	if CoD.isWIIU then
		MainLobbyButtonPane.body.controlsButton = MainLobbyButtonPane.body.buttonList:addButton("CONFIGURAÇÕES DE CONTROLE", nil, 9)
		MainLobbyButtonPane.body.controlsButton.hintText = "Ajuste as configurações do seu controle."
...
CoD.MainLobby.Back = function (MainLobbyWidget, ClientInstance)
	...
	elseif UIExpression.AloneInPartyIgnoreSplitscreen(ClientInstance.controller, 1) == 0 then
		...
		if not CoD.isPartyHost() then
			CustomLeaveMessage.titleText = "SAIR DA SALA?"
			CustomLeaveMessage.messageText = "Você sairá do grupo atual."
			table.insert(CustomLeaveMessage.params, {
				leaveHandler = CoD.MainLobby.ClientLeave,
				leaveEvent = "client_leave",
				leaveText = "SAIR DO GRUPO",
				debugHelper = "You're a client of a private party, remove you from the party"
			})
		else
			CustomLeaveMessage.titleText = "DISSOLVER O GRUPO?"
			CustomLeaveMessage.messageText = "Você é o anfitrião. Dissolver o grupo removerá todos os jogadores."
			table.insert(CustomLeaveMessage.params, {
				leaveHandler = CoD.MainLobby.GoBack,
				leaveEvent = "host_leave",
				leaveText = "SAIR E DISSOLVER",
...
CoD.MainLobby.AddLobbyPaneElements = function (LobbyPane, MenuParty)
	...
	LobbyPane.body.lobbyList.joinableList.maxRows = CoD.MaxPlayerListRows - 2
		LobbyPane.body.lobbyList.joinableList.statusText = "AMIGOS JOGANDO"
...
CoD.MainLobby.ButtonPromptJoin = function (MainLobbyWidget, ClientInstance)
	if UIExpression.IsGuest(ClientInstance.controller) == 1 then
		...
		f39_local0:setMessage("Convidados não podem usar este recurso.")
...
LUI.createMenu.MainLobby = function (LocalClientIndex)
	local MainLobbyName = CoD.isZombie and "ZUMBIS" or "MULTIPLAYER"
	...
	CoD.MainLobby.AddLobbyPaneElements(MainLobbyWidget.lobbyPane, "GRUPO")
