...
LUI.createMenu.UnlockAllPopup = function (f9_arg0)
	...
	CoD.PopupMenus.SetupConfirmPopup(UnlockAllPopup, "DESBLOQUEAR TUDO", "Tem certeza que deseja desbloquear tudo?", CoD.PopupMenus.UnlockAll, CoD.PopupMenus.ClosePopupAfterConfirmation)
...
LUI.createMenu.ResetStatsPopup = function (f9_arg0)
	...
	CoD.perController[f9_arg0].firstTime = true
	CoD.PopupMenus.SetupConfirmPopup(ResetStatsPopup, "REDEFINIR ESTATÍSTICAS", "Tem certeza que deseja redefinir suas estatísticas?", CoD.PopupMenus.ResetStats, CoD.PopupMenus.ClosePopupAfterConfirmation)
...
CoD.PartyLobby.Button_Back = function (f3_arg0, ClientInstance)
	...
	elseif UIExpression.AloneInPartyIgnoreSplitscreen(ClientInstance.controller, 1) == 0 then
		...
		if CoD.isPartyHost() then
			f3_local0.titleText = "SAIR DA SALA?"
			f3_local0.messageText = "Você é o anfitrião. O que deseja fazer com o grupo?"
			table.insert(f3_local0.params, {
				leaveHandler = CoD.PartyLobby.BackAlone,
				leaveEvent = "confirm_leave_alone",
				leaveText = "SAIR SEM O GRUPO",
	...
	elseif Engine.GameModeIsMode(CoD.GAMEMODE_PRIVATE_MATCH) == true and CoD.isPartyHost() then
		local f3_local0 = {
			params = {},
			titleText = "SAIR DA SALA?",
			messageText = "Você é o anfitrião. O que deseja fazer com o grupo?"
		}
		table.insert(f3_local0.params, {
			leaveHandler = CoD.PartyLobby.BackAlone,
			leaveEvent = "confirm_leave_alone",
...
CoD.PartyLobby.OpenPlaylistSelection = function (f5_arg0, ClientInstance)
	if UIExpression.IsGuest(ClientInstance.controller) == 1 then
		...
		f5_local0:setMessage("Convidados não podem usar este recurso.")
	...
	if f19_arg0.body.dlcWarningContainer ~= nil then
		...
		if f19_local0 and Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) == false then
			...
			if f19_local0 == "me" then
				if f19_local1 then
					f19_local2 = "FALTAM MAPAS PARA VOCÊ"
				else
					f19_local2 = "Você não possui todo o conteúdo para esta lista. A partida não poderá ser iniciada."
				end
			elseif f19_local0 == "partymember" then
				if f19_local1 then
					f19_local2 = "FALTAM MAPAS PARA OUTROS"
				else
					f19_local2 = "Jogadores no grupo não possuem todo o conteúdo."
				end
			end
		else
...
CoD.PartyLobby.PopulateButtons_Multiplayer = function (PartyLobbyButtonPane)
	PartyLobbyButtonPane.body.createAClassButton = PartyLobbyButtonPane.body.buttonList:addButton("CRIAR CLASSE", nil, 1)
	...
	PartyLobbyButtonPane.body.rewardsButton = PartyLobbyButtonPane.body.buttonList:addButton("SEQÜÊNCIAS DE PONTOS", nil, 2)
	...
	PartyLobbyButtonPane.body.barracksButton = PartyLobbyButtonPane.body.buttonList:addButton("QUARTEL", nil, 4)
	...
	PartyLobbyButtonPane.body.unlockAllButton = PartyLobbyButtonPane.body.buttonList:addButton("DESBLOQUEAR TUDO", nil, 7)
	PartyLobbyButtonPane.body.unlockAllButton:registerEventHandler("button_action", CoD.PartyLobby.Button_UnlockAll)
	PartyLobbyButtonPane.body.resetStatsButton = PartyLobbyButtonPane.body.buttonList:addButton("REDEFINIR ESTATÍSTICAS", nil, 9)
...
CoD.PartyLobby.PopulateButtons_Zombies = function (f21_arg0)
	f21_arg0.body.barracksButton = f21_arg0.body.buttonList:addButton("CLASSIFICAÇÕES", nil, 2)
...
CoD.PartyLobby.new = function (f29_arg0, f29_arg1, f29_arg2)
	local PartyLobbyWidget = CoD.Lobby.New(f29_arg1, f29_arg0, nil, f29_arg2, "GRUPO", true)
