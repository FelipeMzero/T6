...
CoD.Class.PrepareClassButtonList = function (LocalClientIndex, IngameMenuWidget)
	...
	if CoD.isZombie == true then
		if Engine.CanPauseZombiesGame() and CoD.canLeaveGame(LocalClientIndex) then
			CoD.Class.AddButton(IngameMenuWidget, "RESUMIR", "soloResumeGame")
		end
	else
		if UIExpression.Team(LocalClientIndex, "name") ~= "TEAM_SPECTATOR" and CoD.IsWagerMode() == false then
			CoD.Class.AddButton(IngameMenuWidget, "ESCOLHER CLASSE", "open_chooseClass", CoD.Class.DisableChooseClass())
		end
	end
	if UIExpression.IsVisibilityBitSet(LocalClientIndex, CoD.BIT_ROUND_END_KILLCAM) == 0 and UIExpression.IsVisibilityBitSet(LocalClientIndex, CoD.BIT_FINAL_KILLCAM) == 0 and CoD.Class.IsChooseTeamAvailable() then
		CoD.Class.AddButton(IngameMenuWidget, "MUDAR DE TIME", "open_chooseTeam", CoD.Class.DisableChooseTeam())
	end
	CoD.Class.AddButton(IngameMenuWidget, "OPÇÕES", "open_options")
	if CoD.canLeaveGame(LocalClientIndex) then
		if CoD.isHost() then
			CoD.Class.AddButton(IngameMenuWidget, "ENCERRAR PARTIDA", "open_endGamePopup")
		else
			CoD.Class.AddButton(IngameMenuWidget, "SAIR DA PARTIDA", "open_endGamePopup")
		end
	end
...
LUI.createMenu.class = function (LocalClientIndex)
	...
	local ClassMenuHeader = CoD.isZombie and "ZUMBIS" or "PAUSA"
	local IngameMenuWidget = CoD.InGameMenu.New("class", LocalClientIndex, ClassMenuHeader)
