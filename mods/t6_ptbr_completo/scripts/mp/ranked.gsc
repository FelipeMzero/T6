...
fix_max_allocation_exploit()
{
	...
	for ( ;; )
	{
		...
		allocs = self GetLoadoutAllocation( class_num );
		if ( allocs <= level.maxAllocation && allocs >= 0 && self classIsValid( class_num ) )
		{
			primarygrenade = self getloadoutitemref( class_num, "primarygrenade" );
			if ( self getloadoutitem( class_num, "primarygrenadecount" ) == 0 || ( primarygrenade != "" && primarygrenade != "weapon_null" && primarygrenade != "weapon_null_mp" ) )
			{
				continue;
			}
		}
...
spectate_player_watcher_fix()
{
	...
	while ( true )
	{
		if ( self.pers["team"] != "spectator" || level.gameended )
		{
			...
			self freezecontrols( level.inprematchperiod || level.gameended );
